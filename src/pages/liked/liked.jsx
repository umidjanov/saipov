...
export default function Liked() {
  ...
  const removeFromLiked = useStore((state) => state.removeFromLiked);

  return (
    <div className="bg-[#f7f7f7] text-gray-900 font-sans min-h-screen">
      <NavbarDefault />
      <div className="max-w-6xl mx-auto px-4 py-12">
        <h1 className="text-2xl font-bold mb-6">{t("liked_title")}</h1>
        {liked.length === 0 ? (
          <p className="text-gray-600">{t("liked_empty")}</p>
        ) : (
